...
// =====================================================
// WHATSAPP V2 - BAILEYS INTEGRATION TABLES
// =====================================================

// Store Baileys auth credentials in PostgreSQL
export const whatsappV2AuthSessions = pgTable("whatsapp_v2_auth_sessions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  companyId: varchar("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }),
  sessionId: text("session_id").notNull(), // e.g., "creds.json", "app-state-sync-key-xxx"
  data: jsonb("data").notNull(), // Credential/key data
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
}, (table) => ({
  companySessionUnique: uniqueIndex("whatsapp_v2_auth_sessions_company_session_unique").on(table.companyId, table.sessionId),
}));

// WhatsApp contacts
export const whatsappV2Contacts = pgTable("whatsapp_v2_contacts", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  companyId: varchar("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }),
  jid: text("jid").notNull(), // WhatsApp JID like "1234567890@s.whatsapp.net"
  name: text("name").notNull(), // Push name or contact name
  businessName: text("business_name"), // Business name (nullable)
  avatarUrl: text("avatar_url"), // Avatar URL (nullable)
  isBusiness: boolean("is_business").notNull().default(false),
  phone: text("phone").notNull(), // Extracted phone number
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
}, (table) => ({
  companyJidUnique: uniqueIndex("whatsapp_v2_contacts_company_jid_unique").on(table.companyId, table.jid),
}));

// Active chats (only with messages)
export const whatsappV2Chats = pgTable("whatsapp_v2_chats", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  companyId: varchar("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }),
  jid: text("jid").notNull(), // Chat JID
  contactId: varchar("contact_id").references(() => whatsappV2Contacts.id, { onDelete: "set null" }), // FK to whatsapp_v2_contacts (nullable)
  title: text("title"), // For groups (nullable)
  isGroup: boolean("is_group").notNull().default(false),
  unreadCount: integer("unread_count").notNull().default(0),
  lastMessageId: varchar("last_message_id"), // Will be set after message is created
  lastMessageTs: bigint("last_message_ts", { mode: "number" }), // CRITICAL: only show chats where this is NOT NULL
  archived: boolean("archived").notNull().default(false),
  mutedUntil: timestamp("muted_until"), // Nullable
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
}, (table) => ({
  companyJidUnique: uniqueIndex("whatsapp_v2_chats_company_jid_unique").on(table.companyId, table.jid),
}));

// Message history
export const whatsappV2Messages = pgTable("whatsapp_v2_messages", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  companyId: varchar("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }),
  chatId: varchar("chat_id").notNull().references(() => whatsappV2Chats.id, { onDelete: "cascade" }),
  messageKey: text("message_key").notNull(), // Baileys message key id
  remoteJid: text("remote_jid").notNull(),
  fromMe: boolean("from_me").notNull(),
  content: text("content"), // Text content (nullable)
  messageData: jsonb("message_data").notNull(), // Full Baileys message object
  messageType: text("message_type").notNull(), // text, image, video, audio, document, sticker
  status: text("status").notNull().default("sent"), // sent, delivered, read, failed
  mediaUrl: text("media_url"), // Nullable
  mediaMimeType: text("media_mime_type"), // Nullable
  timestamp: bigint("timestamp", { mode: "number" }).notNull(), // Message timestamp
  createdAt: timestamp("created_at").notNull().defaultNow(),
}, (table) => ({
  chatTimestampIdx: index("whatsapp_v2_messages_chat_timestamp_idx").on(table.chatId, table.timestamp),
  companyMessageKeyUnique: uniqueIndex("whatsapp_v2_messages_company_message_key_unique").on(table.companyId, table.messageKey),
}));

// =====================================================
// WHATSAPP V2 - INSERT SCHEMAS
// =====================================================

export const insertWhatsappV2AuthSessionSchema = createInsertSchema(whatsappV2AuthSessions).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertWhatsappV2ContactSchema = createInsertSchema(whatsappV2Contacts).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertWhatsappV2ChatSchema = createInsertSchema(whatsappV2Chats).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertWhatsappV2MessageSchema = createInsertSchema(whatsappV2Messages).omit({
  id: true,
  createdAt: true,
});

// =====================================================
// WHATSAPP V2 - TYPE EXPORTS
// =====================================================

export type InsertWhatsappV2AuthSession = z.infer<typeof insertWhatsappV2AuthSessionSchema>;
export type SelectWhatsappV2AuthSession = typeof whatsappV2AuthSessions.$inferSelect;

export type InsertWhatsappV2Contact = z.infer<typeof insertWhatsappV2ContactSchema>;
export type SelectWhatsappV2Contact = typeof whatsappV2Contacts.$inferSelect;

export type InsertWhatsappV2Chat = z.infer<typeof insertWhatsappV2ChatSchema>;
export type SelectWhatsappV2Chat = typeof whatsappV2Chats.$inferSelect;

export type InsertWhatsappV2Message = z.infer<typeof insertWhatsappV2MessageSchema>;
export type SelectWhatsappV2Message = typeof whatsappV2Messages.$inferSelect;
