...
- **Per-Policy Data Isolation (Nov 2025):** Each policy maintains independent data for payment methods, documents, notes, consent forms, and reminders. Data is strictly scoped to the specific policy ID, ensuring no cross-contamination between policies even for the same client. This enforces privacy compliance and prevents data leakage across policy years. Previously implemented cross-policy sharing via `getCanonicalPolicyIds()` was removed to address privacy concerns.
