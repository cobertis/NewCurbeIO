...
- **User Profile Page:** Comprehensive profile page at `/users/:id` with inline editing organized into 4 card-based sections: (1) Personal Information (avatar URL, first/last name, email), (2) Contact & Location (phone, address, timezone, language, date of birth), (3) Account Details (role, company, status, creation date), (4) Notification Preferences (email/SMS subscriptions, notifications, invoice alerts). Each section has independent form validation using Zod schemas, proper loading states, and saves changes via PATCH to existing user endpoint. Profile menu in header navigates directly to current user's profile page.
