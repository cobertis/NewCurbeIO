...
-   **Quotes Management System:** A comprehensive insurance quote management system with a streamlined 3-step wizard interface for creating and managing client quotes across 11 insurance product types. Features include visual step indicators, automated effective date defaults (1st of next month), integrated Google Places Address Autocomplete with automatic county detection, and multi-tenant isolation. **ID System (2025-10-21):** Uses 8-character short IDs (alphanumeric, uppercase, excluding confusing characters like O/0, I/1/L) instead of UUIDs for better usability and cleaner URLs. IDs are generated server-side using `generateShortId()` from `server/id-generator.ts` with uniqueness validation.
    -   **Wizard Structure:** 
        ...
        -   Step 3 (Family Members): Simplified to capture ONLY spouses and dependents. **Updated (2025-10-21):** Removed household income and family group size fields as these are no longer collected during quote creation. The step now focuses exclusively on adding family members (spouses/dependents) with dedicated schemas. Uses consistent grid layouts (md:grid-cols-4 for names), section headers with icons. Spouse cards include: First/Middle/Last/Second Last Name, DOB, SSN (auto-formatted), Gender, Phone, Email. Dependent cards additionally include required "Relation" field (Child, Parent, Sibling, Other). After completing the wizard, the user is automatically redirected to the newly created quote's detail page (/quotes/:id) instead of the quotes list.
