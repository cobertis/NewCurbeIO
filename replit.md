...
- **Business Profile Page:** Comprehensive business profile at `/business-profile` accessible via Profile menu in header dropdown. Organized into 4 card-based sections with inline editing: (1) General Information (business logo, friendly/legal names, email, phone, domain, website, niche, currency, API key with generate/copy buttons), (2) Business Physical Address (street, city, postal code, state, country, timezone, platform/outbound languages), (3) Business Information (business type, industry, registration ID type/number, "not registered" checkbox, regions of operation checkboxes), (4) Authorized Representative (first/last name, email, job position, phone). Each section has independent Zod validation, loading states, and saves via PATCH to /api/companies/:id. API key generation uses browser-safe Web Crypto API (window.crypto.getRandomValues).
