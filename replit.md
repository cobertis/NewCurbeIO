...
- **Voice/Voicemail Adapter:** Provides a `VoiceAdapter` interface for campaign voice calls and voicemail drops, with `TelnyxVoiceAdapter` integrating with Telnyx Call Control API and a `MockVoiceAdapter` for testing. Supports asset-based voicemail drops with smart retry logic: jobs with `assetId` wait for assets in `draft`/`generating` status (10-minute intervals, max 6 retries over 1 hour), fail immediately for `failed` assets, and proceed normally when `ready`. Emits `VOICEMAIL_FAILED` events with `final:false` for retry attempts and `final:true` for terminal failures.
