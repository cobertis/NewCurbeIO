...
-   **BulkVS Chat System:** WhatsApp-style SMS/MMS messaging platform with dedicated phone numbers per user, real-time updates, full privacy isolation, and automated webhook management.
    ...
    -   **Webhook System:** Automated webhook creation and assignment during number provisioning/reactivation. Simplified webhook URLs: `{domain}/{company-slug}/{webhook-token}`. Auto-generated secure 32-character tokens uniquely identify each webhook. Domain auto-detection (dev: REPLIT_DOMAINS, prod: app.curbe.io). Two-step process: (1) Create webhook via PUT /webHooks, (2) Configure number with single POST /tnRecord call that sets SMS, MMS, Webhook, CNAM, Campaign, and Call Forward all at once. Webhooks are automatically deleted when phone numbers are deactivated/cancelled.
