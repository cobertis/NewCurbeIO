...
- **Cross-Policy Data Sharing System (Nov 2025):** Documents, notes, reminders, consent forms, and payment methods automatically shared across ALL policies of the same client using SSN (9 digits) or email identification. Implemented with request-scoped memoization via `getCanonicalPolicyIds()` helper that aggregates data from up to 20 policies per client. When viewing any policy, users see all related client data regardless of policy year.
- **CMS Marketplace Integration (Nov 2025):** Separate payload builders for quotes vs policies. `buildCMSPayloadFromPolicy()` enforces exactly one applicant (policy owner/client) with spouses marked as non-applicants (`aptc_eligible: false`, `has_mec: true`) to comply with CMS API requirements. Prevents "household tiene dos applicant" error by explicitly controlling applicant flags instead of trusting database values.
