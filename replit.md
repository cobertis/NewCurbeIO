...
- **iMessage Integration (BlueBubbles):** Modular feature system allowing companies to enable iMessage messaging through BlueBubbles bridge. Features include: (1) Superadmin-only configuration panel for BlueBubbles server settings (serverUrl, password) accessible via Companies page → Manage Features → gear icon on iMessage feature, (2) **Company-specific webhook URLs** (`/api/imessage/webhook/:companySlug`) for scalable multi-tenant architecture where each company has a unique webhook endpoint based on their slug, (3) Secure webhook-based message synchronization with HMAC validation using company-specific webhook secrets, (4) Multi-tenant data isolation with company-scoped message GUIDs, (5) Feature gating on all routes ensuring only enabled companies can access iMessage functionality, (6) Zod validation for settings updates requiring serverUrl/password when enabling, (7) Real-time message delivery with status tracking (sent/delivered/read), (8) BlueBubbles client abstraction (server/bluebubbles.ts) for REST API integration, (9) Support for both iMessage (blue) and SMS/RCS (green) message types. Security: Webhook secrets never exposed to frontend, superadmin-only access to configuration (company admins can only use messaging, not configure), activity logging for settings changes. Backend endpoints accept `companyId` query parameter for superadmin to configure any company. Database schema includes `imessage_conversations` and `imessage_messages` tables with optimized indexes for conversation listing and message pagination.
