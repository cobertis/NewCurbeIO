...
- **Unified Contacts Directory:** Comprehensive contact management system aggregating contacts from Quotes and Policies only (excludes BulkVS SMS contacts and system users/employees). SMS contacts are managed separately in the Chat interface. Features intelligent deduplication with centralized displayName generation (priority: firstName+lastName > companyName > null), advanced filtering, search functionality, CSV export (with SSN masking for non-superadmins), and role-based access control. The system uses a helper function to ensure consistent name formatting across all contact sources, with empty string normalization and recalculation during deduplication merges.
