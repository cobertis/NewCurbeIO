...
- **Communication Systems:**
    ...
    - **WhatsApp Web Integration:** Comprehensive WhatsApp Web feature parity using whatsapp-web.js v1.34.2 with 165 service functions and 169 REST endpoints:
        ...
        - **Profile Picture Proxy:** Server-side endpoint that fetches fresh profile picture URLs from WhatsApp on each request with 30-second cache TTL. Solves issue of expiring WhatsApp CDN URLs by always serving binary images through `/api/whatsapp/contacts/:contactId/avatar`. Returns 204 when no picture available (privacy settings or not set).
        - **Media Lightbox:** Images and videos open in floating modal instead of new browser tab, with autoplay for videos and download button.
        ...
        - **Real-Time Notifications:** Incoming WhatsApp messages trigger bell notifications for all company users with message preview and direct link to conversation. Notifications include sender name, message preview (with media type indicators), and group context if applicable. Notification links use wouter's `useSearch()` hook to detect query parameter changes (`?chat=xxx`) for seamless navigation to specific chats, even when already on the WhatsApp page.
        - **Performance Optimizations:** MessageItem component is memoized with `React.memo()` and custom comparison function to prevent audio/video player restart when typing in chat input. Comprehensive comparison covers all render-relevant fields: core properties (id, ack, isStarred, body, type, hasMedia, timestamp, isForwarded, isFromMe), media URLs (mediaUrl, _blobUrl for optimistic uploads), location data, reactions array, quoted messages, and vCards. Callback functions are ignored as they change on every render but don't affect message display.
