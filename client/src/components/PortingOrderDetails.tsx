...
export function PortingOrderDetails({ order, open, onOpenChange }: PortingOrderDetailsProps) {
  ...
  const { data: commentsData, isLoading: loadingComments } = useQuery<{ comments: Comment[] }>({
    ...
    enabled: !!order?.id && open && activeTab === "communications",
  });
  ...
  const { data: documentsData, isLoading: loadingDocs } = useQuery<{ documents: any[] }>({
    ...
    enabled: !!order?.id && open && activeTab === "requirements",
  });
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 overflow-hidden flex flex-col">
          <div className="border-b px-6">
            <TabsList className="h-auto p-0 bg-transparent gap-6">
              <TabsTrigger 
                value="details" 
                className="px-0 pb-3 pt-4 rounded-none border-b-2 border-transparent data-[state=active]:border-slate-900 data-[state=active]:bg-transparent data-[state=active]:shadow-none"
