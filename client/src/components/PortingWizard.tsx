...
const WIZARD_STEPS: { id: WizardStep; label: string; number: number }[] = [
  { id: 'enter-numbers', label: 'Enter Numbers', number: 1 },
  { id: 'check-portability', label: 'Check Portability', number: 2 },
  { id: 'create-order', label: 'Create Order', number: 3 },
  { id: 'end-user-info', label: 'End User Info', number: 4 },
  { id: 'upload-documents', label: 'Upload Documents', number: 5 },
  { id: 'select-foc-date', label: 'Select FOC Date', number: 6 },
...
export function PortingWizard({ open, onOpenChange, onOrderCreated }: PortingWizardProps) {
  ...
  const handleEnterNumbersNext = () => {
    ...
    if (numbers.length === 0) {
      toast({
        title: 'Invalid Input',
        description: 'Please enter at least one valid phone number in E.164 format',
        variant: 'destructive',
      });
      return;
    }
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Enter phone numbers in E.164 format (e.g., +15551234567) or standard US format. 
  ...
  const renderCheckPortability = () => (
    <div className="space-y-4">
      <div className="text-sm text-muted-foreground mb-4">
        {portableNumbers.length} of {portabilityResults.length} numbers are portable
      </div>
      ...
      {portableNumbers.length === 0 && (
        <div className="p-4 bg-destructive/10 border border-destructive/20 rounded-lg">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            None of the entered numbers can be ported at this time. Please go back and try different numbers.
          </p>
        </div>
      )}
    </div>
        ...
        <div className="space-y-4">
          <div className="p-4 bg-muted/50 rounded-lg space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm">{portingOrder.created_at ? format(new Date(portingOrder.created_at), 'PPp') : 'Just now'}</span>
            </div>
          </div>

          <div className="p-3 bg-muted/30 rounded-lg">
            <div className="text-sm font-medium mb-2">Numbers to Port:</div>
  ...
  const renderSelectFocDate = () => (
    <div className="space-y-4">
      <div className="p-4 bg-muted/30 rounded-lg">
        ...
        <p className="text-sm text-muted-foreground">
          The FOC (Firm Order Commitment) date is when your numbers will be ported to our system.
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 'enter-numbers':
        return renderEnterNumbers();
      case 'check-portability':
        return renderCheckPortability();
      case 'create-order':
        return renderCreateOrder();
      case 'end-user-info':
        return renderEndUserInfo();
      case 'upload-documents':
        return renderUploadDocuments();
      case 'select-foc-date':
        return renderSelectFocDate();
      case 'review-submit':
    ...
    <Dialog open={open} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Step {currentStepIndex + 1} of {WIZARD_STEPS.length}: {getStepTitle()}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-center justify-between pt-4 border-t">
          ...
          >
            ...
            {currentStep === 'review-submit' ? 'Submit Order' : 'Next'}
