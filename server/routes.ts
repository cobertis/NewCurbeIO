...
export async function registerRoutes(app: Express, sessionStore?: any): Promise<Server> {
  ...
  // POST /api/quotes/:id/submit-policy - Convert quote to policy
  app.post("/api/quotes/:id/submit-policy", requireActiveCompany, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!quote.selectedPlan) {
        return res.status(400).json({ message: "Cannot submit quote as policy - no plan selected" });
      }
      ...
      try {
        ...
        const notificationMessage = `A new policy has been created from quote ${quoteId} for client ${clientName}`;
        const notificationLink = `/policies/${policy.id}`;
      ...
      res.status(201).json({ policyId: policy.id });
    } catch (error: any) {
      console.error("Error submitting quote as policy:", error);
      res.status(500).json({ message: error.message || "Failed to submit quote as policy" });
    }
  });
  ...
  app.get("/api/policies", requireActiveCompany, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (currentUser.role === "superadmin") {
        ...
        // Regular users see only their company's policies
        policies = await storage.getPoliciesByCompany(currentUser.companyId);
      }
  ...
  // POST /api/policies/:policyId/documents/upload - Upload a new document
  ...
  // GET /api/policies/:policyId/documents/:documentId/download - Download a document
  ...
  // DELETE /api/policies/:policyId/documents/:documentId - Delete a document
  ...
  // GET /api/policies/:policyId/reminders - List all reminders for a policy
  ...
  // GET /api/policies/:policyId/reminders/:reminderId - Get a specific reminder
  ...
  // POST /api/policies/:policyId/reminders - Create a new reminder
  ...
  // PUT /api/policies/:policyId/reminders/:reminderId - Update a reminder
  ...
  // DELETE /api/policies/:policyId/reminders/:reminderId - Delete a reminder
  ...
  // PUT /api/policies/:policyId/reminders/:reminderId/complete - Mark reminder as completed
  ...
  // PUT /api/policies/:policyId/reminders/:reminderId/snooze - Snooze reminder
  ...
  // POST /api/policies/:id/consents/generate - Generate new consent document
  ...
  // GET /api/policies/:id/consents - List all consents for a policy
  ...
  app.get("/api/policies/:id/marketplace-plans", requireActiveCompany, async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.error("Error fetching marketplace plans:", error);
      res.status(500).json({ message: error.message || "Failed to fetch marketplace plans" });
    }
  });
