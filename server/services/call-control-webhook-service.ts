...
export class CallControlWebhookService {
  ...
  private async routeToQueue(callControlId: string, companyId: string, queueId: string): Promise<void> {
    ...
    const availableAgents = await pbxService.getAvailableAgentsForQueue(companyId, queueId);
    if (availableAgents.length === 0) {
      await this.speakText(callControlId, "All agents are currently busy. Please leave a message after the tone.");
      await this.routeToVoicemail(callControlId, companyId);
      return;
    }
    ...
    if (!agentUser) {
      await this.speakText(callControlId, "No agent is available. Please try again later.");
      await this.hangupCall(callControlId, "NORMAL_CLEARING");
      return;
    }

    await this.speakText(callControlId, "Connecting you to an agent. Please hold.");
    await this.dialSipUri(callControlId, companyId, agentUser);
  }
